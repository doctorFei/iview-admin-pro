...
class Region {
  ...
  playPause(end) {
    ...
    // 当音频处于播放中的时候，若不在相应的region中，则直接跳至相应的region中播放，在其中则暂停
    if (this.wavesurfer.isPlaying()) {
      if ((currentTime > this.start) && (currentTime < this.end)) {
        this.wavesurfer.pause();
      } else {
        this.play(undefined, end);
      }
    } else if ((currentTime > this.start) && (currentTime < this.end)) {
      // 不播放状态，判断是否在区间内，在区间播放
      this.play(currentTime, end);
    } else {
      // 不在区间，直接从区间开始处播放
      this.play(undefined, end);
    }
  ...
  play(start, end) {
    this.wavesurfer.play(start || this.start, end || this.end);
    this.fireEvent('play');
  ...
  playLoop() {
    // this.wavesurfer.un('out', () => {
    //   this.playLoop();
    // });
    // this.playPause(this.end + 0.05);
    this.playPause(this.end);
    this.once('out', () => {
      this.playLoop();
    });
  }
  ...
  render() {
    ...
    regionEl.setAttribute('data-start', this.start);
    ...
    const label = regionEl.appendChild(
      document.createElement('span'),
    );

    label.className = 'region-label';

    label.innerText = document.getElementsByClassName('wavesurfer-region').length + 1;
  ...
  updateRender() {
    ...
    if (this.element != null) {
      ...
      this.element.setAttribute('data-start', this.start);
